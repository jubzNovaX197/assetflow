...
export async function POST(request: Request) {
  ...
  const { employeeCode, name, email, department, designation, phone, isActive } = data;

  if (typeof employeeCode !== 'string' || employeeCode.trim().length === 0) {
    return NextResponse.json(
      {
        status: 'error',
        message: 'employeeCode is required and must be a non-empty string',
      },
      { status: 400 }
    );
  }
  ...
  if (designation !== undefined) {
    validatedData.designation = designation;
  }
  ...
  if (isActive !== undefined) {
    validatedData.isActive = isActive;
  }

  try {
    const employee = await db.orm.public.Employee.create(validatedData);
