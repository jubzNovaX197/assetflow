...
export async function PATCH(
  ...
  function isNullable(field: (typeof NULLABLE_FIELDS)[number], value: unknown) {
    return NULLABLE_FIELDS.includes(field) && value === null;
  }

  if (
    assetTag !== undefined &&
    (typeof assetTag !== 'string' || assetTag.trim().length === 0)
  ) {
    return NextResponse.json(
      {
        status: 'error',
        message: 'assetTag must be a non-empty string when provided',
      },
      { status: 400 }
    );
  }
  ...
  if (
    ...
    (typeof category !== 'string' ||
      !ASSET_CATEGORIES.includes(category as (typeof ASSET_CATEGORIES)[number]))
  ) {
    return NextResponse.json(
      {
        status: 'error',
  ...
  if (
    ...
    (typeof condition !== 'string' ||
      !ASSET_CONDITIONS.includes(condition as (typeof ASSET_CONDITIONS)[number]))
  ) {
    return NextResponse.json(
      {
        status: 'error',
  ...
  if (
    ...
    (typeof status !== 'string' ||
      !ASSET_STATUSES.includes(status as (typeof ASSET_STATUSES)[number]))
  ) {
    return NextResponse.json(
      {
        status: 'error',
  ...
  if (
    ...
    return NextResponse.json(
      {
        status: 'error',
        message: 'manufacturer must be a string or null if provided',
      },
      { status: 400 }
    );
  }

  if (model !== undefined && !isNullable('model', model) && typeof model !== 'string') {
    return NextResponse.json(
      {
        status: 'error',
        message: 'model must be a string or null if provided',
      },
      { status: 400 }
    );
  }
  ...
  if (
    ...
    return NextResponse.json(
      {
        status: 'error',
        message: 'serialNumber must be a string or null if provided',
      },
      { status: 400 }
    );
  }
  ...
  if (
    ...
    return NextResponse.json(
      {
        status: 'error',
        message: 'licenseKey must be a string or null if provided',
      },
      { status: 400 }
    );
  }
  ...
  if (
    ...
    return NextResponse.json(
      {
        status: 'error',
        message: 'purchasePrice must be a valid non-negative number if provided',
      },
      { status: 400 }
    );
  }
  ...
  if (
    ...
    (typeof warrantyExpiry !== 'string' || Number.isNaN(Date.parse(warrantyExpiry)))
  ) {
    return NextResponse.json(
      {
        status: 'error',
  ...
  if (
    ...
    (typeof licenseExpiry !== 'string' || Number.isNaN(Date.parse(licenseExpiry)))
  ) {
    return NextResponse.json(
      {
        status: 'error',
  ...
  try {
    ...
    const isUniqueConflict =
      message.includes('unique') || message.includes('duplicate') || message.includes('conflict');

    if (isUniqueConflict) {
      return NextResponse.json(
        {
          status: 'error',
          message: 'An asset with this assetTag already exists',
        },
        { status: 409 }
      );
    }
